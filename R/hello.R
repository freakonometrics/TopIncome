...
#' @param mu a number (the lower bound, default is 1)
#' @param alpha a number (the power index)
#' @param xi a number (the inverse of \code{alpha}, corresponding to the tail index)
...
dpareto1 <- function (x, mu=1, alpha=1/xi, xi=1/alpha)  { alpha*mu^alpha/x^(alpha+1) }
...
#' @param mu a number (the lower bound, default is 1)
#' @param alpha a number (the power index)
#' @param xi a number (the inverse of \code{alpha})
#' @return the c.d.f. of the Pareto 1 distribution at points \code{x}
#' @examples
#' ppareto1(2, 1, 1.5)
ppareto1 <- function (x, mu=1, alpha=1/xi, xi=1/alpha)  { 1 - ( x/mu )^(-alpha) }
...
qpareto1 <- function (p, mu, alpha=1/xi, xi=1/alpha)  { mu*(1-p)^(-1/alpha) }
...
#' estim
MLE.pareto1 <- function(data, weights=rep(1,length(data)), threshold=min(data))
{
...
#' estim$par.ests
MLE.gpd <- function (data, weights=rep(1,length(data)),
                     threshold = NA, nextremes = NA, method="ml",
...
Top_Share <- function(data, p=.01, q=.1, method="edf", epd.direct=TRUE) {
  ...
  if(method=="epd") {
    for(s in 1:length(q)){
      ...
      up=as.numeric(up)
      datap=data[x<up,]
      xp = xq = Hmisc::wtd.mean(datap[,1], weights=datap[,2])
...
#' @title tidy_income
#' @description Convert income/wealth data into a datafrane that can be used with \code{TopShare} or \code{Pareto_diagram}
#'
